...
export function MapModeToggle({ mode, onModeChange, speciesLoading = false }: Props) {
  return (
    <div className="shrink-0 flex gap-2 px-3 py-2 border-b border-slate-200 bg-white overflow-x-auto">
      <button className={`${btnBase} ${mode === 'single'  ? btnActive : btnInactive}`} onClick={() => onModeChange('single')}>Only this sighting</button>
      ...
      <button className={`${btnBase} ${mode === 'all'     ? btnActive : btnInactive}`} onClick={() => onModeChange('all')}>All sightings</button>
    </div>
  );
}
