...
import {
  ...
  getSentInvites, getInviteByToken, sendInvite, acceptInvite,
} from '../../lib/friends';
...
export function FriendsScreen({ userId, userEmail, inviteToken, onBack, onViewFriendLifeList }: Props) {
  const [profile, setProfile]           = useState<Profile | null>(null);
  const [friends, setFriends]           = useState<Friend[]>([]);
  const [sentInvites, setSentInvites]   = useState<SentInvite[]>([]);
  const [loading, setLoading]           = useState(true);

  // Pending invite from URL param (passed in via prop from App)
  ...
  // Load invite details from token passed in from App
  useEffect(() => {
    if (!inviteToken) return;
    getInviteByToken(inviteToken).then(inv => { if (inv) setPendingInvite(inv); });
  }, [inviteToken]);
  ...
  async function handleAccept() {
    ...
    const result = await acceptInvite(pendingInvite.token, userId);
    if (result.ok) {
      sessionStorage.removeItem('pendingInvite');
      setInviteMsg(`You are now friends with ${pendingInvite.fromDisplayName}!`);
        ...
        <div className="overflow-y-auto flex-1 space-y-4 pb-8">
          ...
          {pendingInvite && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
              {pendingInvite.fromUserId === userId ? (
                <p className="text-slate-700 text-sm">
                  This invite was sent from the account that is currently signed in. Log out and log
                  in as <strong>{pendingInvite.toEmail}</strong> to accept it. If{' '}
                  <strong>{pendingInvite.toEmail}</strong> does not already have an account, then you
                  will need to create one.
                </p>
              ) : (
                <>
                  <p className="text-slate-700 font-medium mb-3">
                    🤝 <strong>{pendingInvite.fromDisplayName}</strong> has invited you to be friends!
                  </p>
