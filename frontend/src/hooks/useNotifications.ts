...
import { fetchNotifications, formatNotificationMessage } from '../lib/notifications';
...
export function useNotifications({
  ...
  async function sendFriendNotification(
    ...
    try {
      ...
      await notifyFriends(type, data, session.access_token);
    } catch { /* best-effort */ }
  }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        payload => { handleNewNotification(payload.new as AppNotification); },
      )
      .subscribe();
